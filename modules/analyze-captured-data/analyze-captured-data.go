...
func AnalyzeDbusData(Telegrams *[]DbusTelegram, TsInterval int64) AnalyzedData {
	...
	for i := 0; i < len(telegrams); i++ {
		...
		if !strings.Contains(telegrams[i].Topic, "timeseries_json_databus") {
			continue
		}

		fmt.Println("Starting analysis with telegram Seq = ", telegrams[i].Payload.Seq)

		for j := 0; j < len(telegrams[i].Payload.Vals); j++ {

			for p := 0; p < len(checkedIds); p++ {
				if telegrams[i].Payload.Vals[j].Id == checkedIds[p] {
					idAlreadyChecked = true
					break
				}
			}
			...
			id = telegrams[i].Payload.Vals[j].Id
			issue.SignalId = fmt.Sprintf("%s - from Telegram Seq number = %d", telegrams[i].Payload.Vals[j].Id, telegrams[i].Payload.Seq)
			tsRFC3339, _ := time.Parse(time.RFC3339, telegrams[i].Payload.Vals[j].Ts)
			ts = tsRFC3339.UnixMilli()
			tsOffset = 200

			for k := i; k < len(telegrams); k++ {

				for l := 0; l < len(telegrams[k].Payload.Vals); l++ {

					if telegrams[k].Payload.Vals[l].Id == id {

						cmptsRFC3339, _ := time.Parse(time.RFC3339, telegrams[k].Payload.Vals[l].Ts)
						cmpts := cmptsRFC3339.UnixMilli()

						if (cmpts - ts) > TsInterval+tsOffset {

							errorFlag = true
							Message := fmt.Sprintf("Missing records near seq: %d", telegrams[k].Payload.Seq)
							issue.Messages = append(issue.Messages, Message)
							fmt.Println("Missing records from id: ", id, "near seq: ", telegrams[k].Payload.Seq)
						}
						ts = cmpts
					}
				}
